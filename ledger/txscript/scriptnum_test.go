// Our scriptNum implementation is based on https://github.com/btcsuite/btcd, which is available under the ISC license.
...
func TestMakeScriptNum(t *testing.T) {
	...
	errNumTooBig := errors.New("number too big")
...
// XXX: This is a terrible hack; btcd's error typing is nicer than ours is.
func tstCheckScriptError(actual, expected error) error {
	if expected == nil {
		if actual != nil {
			return fmt.Errorf("expected no error, but got: %v", actual)
		}
	} else {
		if actual == nil {
			return fmt.Errorf("got no error, but expected: %v", expected)
		}
	}
	return nil
}
