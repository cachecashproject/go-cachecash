...
func testBlocksExists(t *testing.T) {
	...
	e, err := BlockExists(ctx, tx, o.Rowid)
	if err != nil {
...
func testBlocksFind(t *testing.T) {
	...
	blockFound, err := FindBlock(ctx, tx, o.Rowid)
	if err != nil {
		t.Error(err)
	}
...
var (
	blockDBTypes = map[string]string{`Rowid`: `integer`, `Height`: `integer`, `BlockID`: `bytea`, `ParentID`: `bytea`, `Raw`: `bytea`}
	_            = bytes.MinRead
)
