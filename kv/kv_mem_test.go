...
func TestKVBasic(t *testing.T) {
	...
	_, err = c.GetInt64("int")
	assert.Equal(t, err, ErrUnsetValue)
	nonce, err = c.SetInt64("int", -1)
	assert.Nil(t, err)
	intOut, err := c.GetInt64("int")
	assert.Nil(t, err)
	...
	_, err = c.GetFloat64("float")
	assert.Equal(t, err, ErrUnsetValue)
	nonce, err = c.SetFloat64("float", -1.2)
	assert.Nil(t, err)
	floatOut, err := c.GetFloat64("float")
	assert.Nil(t, err)
	...
	_, err = c.GetString("str")
	assert.Equal(t, err, ErrUnsetValue)
	nonce, err = c.SetString("str", "hello")
	assert.Nil(t, err)
	strOut, err := c.GetString("str")
	assert.Nil(t, err)
