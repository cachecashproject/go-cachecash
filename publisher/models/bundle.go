...
var BundleWhere = struct {
	...
	RequestSequenceNo: whereHelperint{field: "\"bundle\".\"request_sequence_no\""},
...
var (
	...
	bundleColumnsWithoutDefault = []string{"raw", "client_public_key", "objectid"}
	bundleColumnsWithDefault    = []string{"id", "escrow_id", "block_id", "request_sequence_no"}
...
func (bundleL) LoadEscrow(ctx context.Context, e boil.ContextExecutor, singular bool, maybeBundle interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(qm.From(`escrow`), qm.WhereIn(`escrow.id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
